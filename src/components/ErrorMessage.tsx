...
class ErrorMessage extends React.Component<Props, {}> {
  ...
  render() {
    const errorEntry = this.props.entries.map((entry: ErrorEntry) =>
      <div className="error-entry" key={entry.id}>
        ...
        <div className="error-info">
          <div className="error-name type type--pos-large-bold" onClick={() => this.viewNode(entry.name)}>{entry.name}</div>
